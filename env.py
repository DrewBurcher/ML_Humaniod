...
from config import (ENV_CONFIG, REWARD_WEIGHTS, ACTUATED_JOINT_INDICES,
                    ARM_JOINT_INDICES, ANKLE_JOINT_INDICES)
...
class T1WalkingEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, reward_weights=None, mask_arms=False):
        ...
        self.arm_indices = ARM_JOINT_INDICES

        # Per-joint PD gains: softer + more damped on ankles to suppress flapping.
        self._pos_gains = [0.2] * self.num_actuated
        self._vel_gains = [0.5] * self.num_actuated
        for i, idx in enumerate(self.actuated_indices):
            if idx in ANKLE_JOINT_INDICES:
                self._pos_gains[i] = 0.1   # half P
                self._vel_gains[i] = 1.0   # double D

        # Physics sub-steps per policy step
        self.sim_steps_per_action = self.cfg["control_freq"] // self.cfg["policy_freq"]

        # â”€â”€ Observation space â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Action space: normalized torques in [-1, 1] â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def set_mask_arms(self, mask):
        """No-op: arms are permanently held at zero (no longer in action space)."""
        pass
    ...
    def _do_reset(self):
        ...
        # Lock arm joints to zero (passive â€” not exposed to the policy)
        if self.arm_indices:
            arm_forces = self.robot.joint_max_torque[self.arm_indices].tolist()
            p.setJointMotorControlArray(
                self.robot.robot, self.arm_indices, p.POSITION_CONTROL,
                targetPositions=[0.0] * len(self.arm_indices),
                positionGains=[0.5] * len(self.arm_indices),
                velocityGains=[1.0] * len(self.arm_indices),
                forces=arm_forces,
                physicsClientId=self.client)
    ...
    def reset(self, seed=None, options=None):
        ...
        for attempt in range(3):
            try:
                ...
                # Server died â€” force full reconnect
                self.client = None
                self.robot = None
                self.plane = None
    ...
    def step(self, action):
        ...
        # â”€â”€ Compute reward â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        reward, reward_info = self._compute_reward(state, applied_torques)

        # â”€â”€ Check termination â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _compute_reward(self, state, torques):
        ...
        # 1. Forward velocity reward â€” linear in v_x, grows without bound.
        ...
        # 6. Z-velocity penalty: penalize downward z velocity â€” max(-z_dot, 0).
        #    Discourages dropping; coefficient is negative so reward is negative.
