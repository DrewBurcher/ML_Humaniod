...
def plot_learning_curve(df, out, smooth_w=50):
    ...
    ax.set_title("Learning Curve â€” Episode Return")
...
def plot_episode_length(df, out, smooth_w=50):
    ...
    ax.set_title("Survival â€” Episode Length Over Training")
    ax.legend(loc="lower right")
    fig.savefig(out)
    plt.close(fig)
...
def plot_component_pie(metrics, out, last_n=200):
    ...
    ax.set_title(f"Reward Composition â€” Last {len(tail)} Episodes")
    fig.savefig(out)
    plt.close(fig)
...
def plot_entropy(metrics, out, smooth_w=200):
    ...
    ax.set_xlabel("Total environment timesteps")
    ax.set_ylabel("Entropy coefficient Î±")
...
def plot_reward_distribution(df, out):
    ...
    ax.set_title("Episode Return Distribution â€” Early vs Late Training")
    ax.legend()
    fig.savefig(out)
    plt.close(fig)
